...
//
    // 定义要发送的回复消息
    //
    //const char* ResponseMessage = "Hi, QUIC Client this is from QUIC Server!";
    size_t ResponseLength = strlen(ResponseMessage);
    
    //
    // Allocates and builds the buffer to send over the stream.
    // 根据实际消息长度分配内存，而不是使用固定的 SendBufferLength
    //
    void* SendBufferRaw = malloc(sizeof(QUIC_BUFFER) + ResponseLength);
    if (SendBufferRaw == NULL) {
        printf("SendBuffer allocation failed!\n");
        MsQuic->StreamShutdown(Stream, QUIC_STREAM_SHUTDOWN_FLAG_ABORT, 0);
        return;
    ...
    // 复制回复消息到缓冲区
    memcpy(SendBuffer->Buffer, ResponseMessage, ResponseLength);
    SendBuffer->Length = (uint32_t)ResponseLength;  // 设置为实际消息长度

    printf("[strm][%p] Sending response: %s\n", Stream, ResponseMessage);
    ...
    )
{
    ...
    case QUIC_STREAM_EVENT_RECEIVE:
        ...
        // Print all received buffers
        for (uint32_t i = 0; i < Event->RECEIVE.BufferCount; i++) {
            QUIC_BUFFER* buffer = &Event->RECEIVE.Buffers[i];
            
            // Print buffer info
            printf("[strm][%p] Buffer %u length: %u bytes\n", 
                   Stream, i, buffer->Length);
            
            // Print as hex
            printf("[strm][%p] Buffer %u (hex): ", Stream, i);
            for (uint32_t j = 0; j < buffer->Length; j++) {
                printf("%02X ", buffer->Buffer[j]);
                // Add newline every 16 bytes for readability
                if ((j + 1) % 16 == 0 && j < buffer->Length - 1) {
                    printf("\n[STRM][%p]                 ", Stream);
                }
            }
            printf("\n");
            ...
            printf("[strm][%p] Buffer %u (text): ", Stream, i);
            for (uint32_t j = 0; j < buffer->Length; j++) {
                char c = (char)buffer->Buffer[j];
                if (c >= 32 && c <= 126) {  // Printable ASCII
                    printf("%c", c);
                } else {
                    printf(".");
                }
                // Add newline every 64 characters for readability
                if ((j + 1) % 64 == 0 && j < buffer->Length - 1) {
                    printf("\n[STRM][%p]                  ", Stream);
                }
            }
            printf("\n");
        ...
        // Notify MsQuic that we've processed the data
        // This allows the receive buffers to be released
        if (Event->RECEIVE.Flags & QUIC_RECEIVE_FLAG_FIN) {
            printf("[strm][%p] FIN received with data\n", Stream);
        }
        break;
    ...
    case QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN:
        ...
        ServerSend(Stream);
        char replymsg[20];
        memset(replymsg,0x00,sizeof(replymsg));
        sprintf(replymsg,"reply from server!");
        //ServerSend_Msg(Stream,replymsg);
        break;
...
{
    QUIC_ADDR_STR LocalAddrStr = { 0 };
    QUIC_ADDR_STR RemoteAddrStr = { 0 };
    QUIC_ADDR LocalAddr, RemoteAddr;
    uint32_t LocalAddrSize = sizeof(LocalAddr), RemoteAddrSize = sizeof(RemoteAddr);
    ...
    case QUIC_CONNECTION_EVENT_CONNECTED:
        ...
        // 获取本端 IP 和端口
        MsQuic->GetParam(Connection, QUIC_PARAM_CONN_LOCAL_ADDRESS, &LocalAddrSize, &LocalAddr);
        // 获取对端 IP 和端口
        MsQuic->GetParam(Connection, QUIC_PARAM_CONN_REMOTE_ADDRESS, &RemoteAddrSize, &RemoteAddr);
       
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        // 将地址信息保存起来，或在此处处理
        printf("[ServerSide][connection:%p] Local:[%s], Remote:[%s],[QUIC_CONNECTION_EVENT_CONNECTED]\n", Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
        break;
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT:
        ...
        if (Event->SHUTDOWN_INITIATED_BY_TRANSPORT.Status == QUIC_STATUS_CONNECTION_IDLE) {
            printf("[ServerSide][connection:%p]Local:[%s],Remote:[%s],[SHUTDOWN_INITIATED_BY_TRANSPORT.Status=QUIC_STATUS_CONNECTION_IDLE]\n", Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
        } else {
            printf("[ServerSide][connection:%p]Local:[%s],Remote:[%s],[SHUTDOWN_INITIATED_BY_TRANSPORT.Status=0x%x]\n",
             Connection,LocalAddrStr.Address,RemoteAddrStr.Address,Event->SHUTDOWN_INITIATED_BY_TRANSPORT.Status);
        }
        break;
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE:
        ...
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        printf("[ServerSide][connection:%p]Local:[%s],Remote:[%s],[QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE]\n", Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
    ...
    case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED:
        ...
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        printf("[ServerSide][connection:%p]Local:[%s],Remote:[%s],[strm:%p][QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address,Event->PEER_STREAM_STARTED.Stream);
    ...
    case QUIC_CONNECTION_EVENT_RESUMED:
        ...
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        printf("[ServerSide][connection:%p]Local:[%s],Remote:[%s],[QUIC_CONNECTION_EVENT_RESUMED]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
        break;
    default:
        break;
    ...
    )
{
    ...
    */ 
    // 方法1：填充字符串
    const char* message = "Hello, QUIC Server this is from QUIC client init!";
    size_t messageLen = strlen(message);

    SendBufferRaw = (uint8_t*)malloc(sizeof(QUIC_BUFFER) + messageLen);
    if (SendBufferRaw == NULL) {
        printf("SendBuffer allocation failed!\n");
        Status = QUIC_STATUS_OUT_OF_MEMORY;
        goto Error;
    ...
    // 复制消息到缓冲区
    memset(SendBuffer->Buffer,0x00,messageLen);
    memcpy(SendBuffer->Buffer, message, messageLen);
    SendBuffer->Length = (uint32_t)messageLen;  // 设置正确的长度

    printf("[strm][%p] Sending data[%.*s]\n", Stream, (int)messageLen, SendBuffer->Buffer);
    ...
    )
{
    ...
    //printf("ClientSend_Msg-[strm][%p] Starting...\n", Stream);
    ...
    */
    // 方法1：填充字符串
    //const char* message = "Hello, QUIC Server this is from QUIC client msg!";
    size_t messageLen = strlen(message);
    ...
    printf("ClientSend_Msg-[strm][%p] Sending data[%.*s]\n", Stream, (int)messageLen, SendBuffer->Buffer);
    ...
    )
{
    ...
    case QUIC_CONNECTION_EVENT_CONNECTED:
        ...
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        printf("[ClientSide][connection:%p]Local:[%s],Remote:[%s],[QUIC_CONNECTION_EVENT_CONNECTED]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
        ...
        for(int i=0;i<5;i++)
        {
            ...
            sprintf(msg,"client msg test-%04d",i);
            ClientSend_Msg(Connection,msg);
            Sleep(1);
        }
        break;
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT:
        ...
        if (Event->SHUTDOWN_INITIATED_BY_TRANSPORT.Status == QUIC_STATUS_CONNECTION_IDLE) {
            printf("[ClientSide][connection:%p]Local:[%s],Remote:[%s],[SHUTDOWN_INITIATED_BY_TRANSPORT=QUIC_STATUS_CONNECTION_IDLE]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
        } else {
            printf("[ClientSide][connection:%p]Local:[%s],Remote:[%s],[SHUTDOWN_INITIATED_BY_TRANSPORT.Status=0x%x]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address,Event->SHUTDOWN_INITIATED_BY_TRANSPORT.Status);
        }
        break;
    case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_PEER:
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE:
        ...
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        printf("[ClientSide][connection:%p]Local:[%s],Remote:[%s],[QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address);
    ...
    case QUIC_CONNECTION_EVENT_RESUMPTION_TICKET_RECEIVED:
        ...
        QuicAddrToString(&LocalAddr, &LocalAddrStr);
        QuicAddrToString(&RemoteAddr, &RemoteAddrStr);
        printf("[ClientSide][connection:%p]Local:[%s],Remote:[%s],[Resumption ticket received (%u bytes):]\n", 
                Connection,LocalAddrStr.Address,RemoteAddrStr.Address,Event->RESUMPTION_TICKET_RECEIVED.ResumptionTicketLength);
